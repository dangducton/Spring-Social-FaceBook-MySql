...
import com.dangducton.securityservice.SercurityService;
...
public class SpringFacebookController {
	...
	public String facebookprofiledata(@PathVariable String accessToken, Model model, HttpServletRequest request) {
		...
		if (dbUser != null) {
			...
			dbUser.setImageUrl("https://graph.facebook.com/" + user.getId() + "/picture");
		...
		sercurityService.autoLogin(user.getEmail(), null, role, request);
